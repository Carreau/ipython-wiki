...
<tr><td> Updated </td><td> May 1, 2013</td></tr>
...
|:---:|:---:|:---|
| ```GET``` | /api/notebooks/foo/bar | returns a dict with the "foo/bar" dashboard's sub-directories, files, notebooks and path ** | 
| ```POST``` | /api/notebooks/foo/bar | if sending a body, saving that body as a new notebook in "foo/bar"; if no body, create a new notebook in "foo/bar" | 
| ```GET``` | /api/notebooks/foo/bar/\<notebook_id\> | returns the json data for the notebook with the given ID in "foo/bar" |
| ```PUT``` | /api/notebooks/foo/bar/\<notebook_id\> | saves an existing notebook with body data in "foo/bar" |
| ```DELETE``` | /api/notebooks/foo/bar/\<notebook_id\> | deletes the notebook with the given ID in "foo/bar" | 
